...
void UpdateProfile::on_updateBtn_clicked()
{
    ...
    if(!CheckEmail())
    {
        CustomMessageBox messageBox("Email mới không hợp lệ!");
        int result = messageBox.exec();
        if (result == QMessageBox::Ok) {
            messageBox.close();
        }
    ...
    {
        CustomMessageBox messageBox("Mật khẩu mới quá yếu!");
        int result = messageBox.exec();
        if (result == QMessageBox::Ok) {
            messageBox.close();
        }
    ...
    {
        CustomMessageBox messageBox("Số điện thoại mới không hợp lệ!");
        int result = messageBox.exec();
        if (result == QMessageBox::Ok) {
            messageBox.close();
        }
    ...
    else
    {
        ...
        {
            CustomMessageBox messageBox("Hồ sơ của bạn đã được cập nhật thành công!");
            int result = messageBox.exec();
            if (result == QMessageBox::Ok) {
                messageBox.close();
            }
