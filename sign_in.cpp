...
void Sign_In::insertData()
{
    ...
    if(sql.exec())
    {
        CustomMessageBox messageBox("Người dùng đã được thêm thành công");
...
void Sign_In::on_signBtn_clicked()
{
    ...
    {
        CustomMessageBox messageBox("Bạn cần nhập đầy đủ thông tin trước");
        int result = messageBox.exec();
        if (result == QMessageBox::Ok) {
            messageBox.close();
        }
    ...
    {
        CustomMessageBox messageBox("Email của bạn không hợp lệ!");
        int result = messageBox.exec();
        if (result == QMessageBox::Ok) {
            messageBox.close();
        }
    ...
    {
        CustomMessageBox messageBox("Mật khẩu của bạn quá yếu!");
        int result = messageBox.exec();
        if (result == QMessageBox::Ok) {
            messageBox.close();
        }
    ...
    {
        CustomMessageBox messageBox("Số điện thoại của bạn không hợp lệ!");
        int result = messageBox.exec();
        if (result == QMessageBox::Ok) {
            messageBox.close();
        }
    ...
    else
    {
        if(!database.open())
        {
            CustomMessageBox messageBox("Kiểm tra kết nối cơ sở dữ liệu của bạn");
            int result = messageBox.exec();
            if (result == QMessageBox::Ok) {
                messageBox.close();
            }
        ...
        if(sql.exec(checkExist))
        {
            if(sql.next() == true)
            {
                CustomMessageBox messageBox("Tên người dùng này đã tồn tại");
                int result = messageBox.exec();
                if (result == QMessageBox::Ok) {
                    messageBox.close();
                }
